...
LLM_PROMPTS = {
    ...
    "TOOL_EXECUTION_MANAGER_PROMPT": """You are a AI tool executor. Given a plan, execute it using the provided APIs. End the conversation with ##TERMINATE##""",

    "PLANNER_PROMPT": """You are a AI planner. Come up with a plan that can be executed by the given APIs.
    The plan should be step-by-step and should be clear and concise. Ideally, the plan should be less then 10 steps.
    Each step starts with a command STEP: and then the command follows. Return '##TERMINATE##' when the task is done.""",
